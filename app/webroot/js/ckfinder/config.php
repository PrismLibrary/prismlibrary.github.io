...
// Pega caminho
$caminhoCompleto = explode("/", $_SERVER['PHP_SELF']);
$arquivo = end($caminhoCompleto);
$arquivoTamanho = strlen($arquivo);
$caminho = substr($_SERVER['PHP_SELF'] ,0 , -$arquivoTamanho);
//$caminhoFinal = substr($caminho ,0 , -32) . '/files/clientes/';

$baseUrl = substr($caminho ,0 , -32) . '/files/material/';
...
if (isset($_SESSION['Auth']['User'])) {
	...
	$baseUrl = $caminhoFinal.$usuarioLogado['caminho'].'/material/';
} else {
	$baseUrl = '';
}
*/
